...
class CudaToolkitConan(ConanFile):
    ...
    def build(self):
        ...
        platform_components = {
            "linux-x86_64": ["libcufile", "cuda_sandbox_dev", "cuda_opencl", "cuda_culibos"],
            "linux-sbsa": ["libcufile", "cuda_sandbox_dev", "cuda_culibos"],
        ...
        for component_name, data in cuda_distrib.items():
            ...
            url = f"{base_url}/{(data[cuda_platform]['cuda13.2'] if component_name == 'cuda_compat' else data[cuda_platform])['relative_path']}"
            checksum = (data[cuda_platform]['cuda13.2'] if component_name == 'cuda_compat' else data[cuda_platform])['sha256']
